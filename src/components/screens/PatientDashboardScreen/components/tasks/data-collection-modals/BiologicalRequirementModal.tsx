...
export function BiologicalRequirementModal({
  task,
  patientId,
  requirementCode,
  ...
  const sections = useMemo(() => {
    const ref = getBiologicalRef(requirementCode);
    if (!ref) return [];
    return convertBiologyDataFieldsToFormConfig([
      { code: ref.code, name: ref.name, unit: ref.unit, availableUnits: ref.availableUnits },
    ]);
  }, [requirementCode]);

  const title = getMeasureLabel(requirementCode, MeasureCategory.BIOLOGICAL);
  ...
  const handleSubmit = (formData: Record<string, unknown>) => {
    ...
    if (!entry) throw new Error('Veuillez saisir une valeur');
    ...
    toast.show('Success', 'Marqueur enregistré');
    onClose();
  };
    ...
    <DataRequirementModalLayout
      ...
      emptyState={
        sections.length === 0
          ? {
              isMissing: true,
              title: 'Référence biologique introuvable',
