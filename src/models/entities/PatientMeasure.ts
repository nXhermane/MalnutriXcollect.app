...
import { AnthropometricDTO, CreatePatientMeasureDTO, DataFieldDTO, PatientMeasureDTO } from "../schemas";
...
export namespace PatientMeasureHelpers {
    ...
    export function addMeasure(patientMeasure: PatientMeasureDTO, newMeasure: AnthropometricDTO): PatientMeasureDTO {
    ...
    export function addField(patientMeasure: PatientMeasureDTO, newField: DataFieldDTO): PatientMeasureDTO {
        if (!canBeModified(patientMeasure)) {
            throw new Error("Cette measure ne peut plus etre modifier")
        }
