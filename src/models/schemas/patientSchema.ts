...
export const contactSchema = v.object({
    ...
    tel: v.optional(v.pipe(
        v.string(),
        v.regex(/^[+]?[\d\s-()]+$/, "Numéro de téléphone invalide")
    ))
});
...
export const addressSchema = v.object({
    fullAddress: v.pipe(
        v.string(),
        v.nonEmpty("L'adresse complète est requise")
    ),
...
export const patientSchema = v.object({
    ...
    birthdate: v.pipe(
        v.string(),
        v.isoTimestamp("Date de naissance invalide")
    ),
